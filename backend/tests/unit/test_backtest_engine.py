...
def test_backtest_engine_agents_respects_execution_allowed(monkeypatch) -> None:
    index = pd.date_range('2025-01-01', periods=180, freq='D')
    trend = np.linspace(1.05, 1.10, len(index))
    noise = 0.001 * np.sin(np.arange(len(index)))
    close = trend + noise
    ...
    engine = BacktestEngine()
    result = engine.run('EURUSD', 'D1', '2025-01-01', '2025-04-30', strategy='agents')

    assert result.metrics.get('strategy') == 'agents_v1'
    assert result.metrics.get('trades') == 0
