...
AGENT_TOOL_DEFINITIONS: dict[str, dict[str, Any]] = {
    'news_search': {
        'label': 'News Search',
        'description': "Normalize, deduplicate and score news batch by symbol relevance via MCP.",
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.news_search',
    },
    'macro_calendar_or_event_feed': {
        'label': 'Macro Event Feed',
        'description': "Filter and score macro-economic events by impact via MCP.",
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.macro_event_feed',
    },
    'symbol_relevance_filter': {
        'label': 'Symbol Relevance Filter',
        'description': "Filter news and macro by relevance threshold for a given symbol via MCP.",
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.symbol_relevance_filter',
    },
    'sentiment_or_event_impact_parser': {
        'label': 'Sentiment Impact Parser',
        'description': "Parse directional sentiment from headlines with asset-class-specific dictionaries via MCP.",
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.sentiment_parser',
    },
    'market_snapshot': {
        'label': 'Market Snapshot',
        'description': 'Normalized market snapshot with derived metrics (spread ratio, candle ratios) via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.market_snapshot',
    },
    'indicator_bundle': {
        'label': 'Indicator Bundle',
        'description': 'Real RSI, EMA, MACD, ATR calculation from OHLC data — no passthrough — via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.indicator_bundle',
    },
    'divergence_detector': {
        'label': 'Divergence Detector',
        'description': 'RSI-price bullish/bearish divergence detection over N bars via MCP.',
        'enabled_by_default': True,
    ...
    'support_resistance_or_structure_detector': {
        ...
        'description': 'S/R level identification by pivot clustering with touch counting via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.support_resistance_detector',
    },
    'pattern_detector': {
        'label': 'Pattern Detector',
        'description': 'Candlestick pattern detection: doji, hammer, engulfing, pin bar, shooting star via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.pattern_detector',
    },
    'multi_timeframe_context': {
        'label': 'Multi Timeframe Context',
        'description': "Multi-TF alignment synthesis with confluence score and dominant direction via MCP.",
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.multi_timeframe_context',
    },
    'market_regime_context': {
        'label': 'Market Regime Detector',
        'description': 'Market regime classification (trending/ranging/volatile/calm) by slope + ATR via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.market_regime_detector',
    },
    'session_context': {
        'label': 'Session Context',
        'description': 'Active market sessions, overlaps and real-time liquidity conditions via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.session_context',
    },
    'correlation_context': {
        'label': 'Correlation Analyzer',
        'description': "Rolling Pearson correlation between two price series with lead/lag analysis via MCP.",
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.correlation_analyzer',
    },
    'volatility_context': {
        'label': 'Volatility Analyzer',
        'description': 'ATR, historical volatility, Bollinger bandwidth, volatility percentile via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.volatility_analyzer',
    },
    'evidence_query': {
        'label': 'Evidence Query',
        'description': 'Agent evidence aggregation and scoring with directional consensus via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.evidence_query',
    },
    'thesis_support_extractor': {
        'label': 'Thesis Support Extractor',
        'description': 'Thesis argument normalization and weighting for debate agents via MCP.',
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.thesis_support_extractor',
    },
    'scenario_validation': {
        'label': 'Scenario Validation',
        'description': "Trading scenario validation with SL/TP geometry and risk/reward ratio via MCP.",
        'enabled_by_default': True,
        'reference_origin': 'mcp_trading_server.scenario_validation',
    },
    'position_size_calculator': {
        'label': 'Position Size Calculator',
        'description': "Asset-class-adapted position size calculation with margin verification via MCP.",
        'enabled_by_default': True,
...
def normalize_external_mcps(raw: object) -> list[dict[str, Any]]:
...
def get_external_tools_for_agent(agent_name: str, settings: dict) -> list[dict[str, Any]]:
