...
Source files covered here:
- `backend/app/services/execution/executor.py`
- `backend/app/services/execution/preflight.py`
- `backend/app/services/agentscope/registry.py` (execution-manager dispatch, lines ~1493–1567)
...
|---|---|---|
| `ALLOW_LIVE_TRADING=false` default | `backend/app/core/config.py:121` | Live trading is opt-in. Must be explicitly set to `true`. |
| `trader-operator` role check | `backend/app/api/routes/runs.py:93` | API layer rejects live-mode run creation from users without `super-admin`, `admin`, or `trader-operator` role. |
...
|---|---|---|
| `METAAPI_TOKEN` | _(empty)_ | MetaAPI API token. Required for paper and live modes. |
| `METAAPI_ACCOUNT_ID` | _(empty)_ | Default MetaAPI account ID. Can be overridden per-run via `metaapi_account_ref`. |
